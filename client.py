...
class JuiceWRLDAPI:
    ...
    def get_random_radio_song(self) -> Dict[str, Any]:
        """Get a random radio song with full metadata from the API.

        This wraps the `/juicewrld/radio/random/` endpoint.
        """
        return self._get('/juicewrld/radio/random/')
    ...
    def play_juicewrld_song(self, song_id: int) -> Dict[str, Any]:
        ...
        It is designed to **never raise** for normal error conditions; instead,
        it returns a dictionary with a ``status`` key indicating what happened.
        """
        try:
            try:
                song_data = self.get_juicewrld_song(song_id)
            except NotFoundError:
                # The song exists in the main catalog but not in the player
                # endpoint, or the ID is invalid for the player API.
                return {
                    'error': 'Song not found in player endpoint (404)',
                    'song_id': song_id,
                    'status': 'not_found',
                }
            except JuiceWRLDAPIError as e:
                # Bubble up API-level issues in a structured way
                return {
                    'error': str(e),
                    'song_id': song_id,
                    'status': 'api_error',
            ...
            if 'file' not in song_data:
                return {
                    'error': 'Song file information not found',
                    'song_id': song_id,
                    'status': 'no_file_info',
                }
            ...
            for test_path in possible_paths:
                try:
                    ...
                    # Ignore individual path test failures and keep trying
                    continue
            ...
            return {
                'error': f'Unexpected request failure: {str(e)}',
                'song_id': song_id,
